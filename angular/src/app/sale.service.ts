...
export class SaleService {
  ...
  constructor(private http: Http) { }
    ...
    // ------
    create(item: string, category: string,
        quality: string, price: number, manufacturer: string): Promise<Sale> {
    return this.http
        .post(this.salesUrl, JSON.stringify({item: item,
            category: category, quality: quality, price: price, manufacturer: manufacturer }), {headers: this.headers})
        .toPromise()
